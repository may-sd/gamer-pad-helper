...
import { CHANNEL_PERMS_COMMAND, HasGuildCommands } from './commands.js'
...
const reminder = () => {
    ...
    channel.send(
        'This voice channel will shut down in 5 minutes (at ' + time + ').'
    )
}

const streamReminder = cron.schedule(
    '55 14 * * 6',
    () => {
        reminder()
    },
...
const streamClose = cron.schedule(
    '0 15 * * 6',
    () => {
        closeChannel()
    },
...
const streamOpen = cron.schedule(
    '0 22 * * 6',
    () => {
        openChannel()
    },
...
const morningOpen = cron.schedule(
    '0 10 * * *',
    () => {
        openChannel()
    },
...
const eveningReminder = cron.schedule(
    '54 23 * * *',
    () => {
        reminder()
    },
...
const eveningClose = cron.schedule(
    '59 23 * * *',
    () => {
        closeChannel()
    },
...
app.post('/interactions', async function (req, res) {
    ...
    if (type === InteractionType.APPLICATION_COMMAND) {
        ...
        if (name === 'channelperms') {
            return res.send({
                // eventually want to check if you're admin, then run command as normal. else it'll return this
            ...
            if (typeof time === 'undefined') {
                ...
                channel.permissionOverwrites
                    .edit(role_id, { [permission]: permission_level })
                    .catch((msg) => {
                        error = msg
                    })
            } else {
                error =
                    "i haven't actually implemented this yet because i didn't want to work with databases. lol. lmao, even"
            }
